...
import { SidebarControls, Mode } from '../components/SidebarControls';
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-8 max-w-6xl mx-auto w-full">
          {/* INPUT FORM SECTION */}
          <div className="bg-slate-900/50 border border-slate-800/80 rounded-2xl p-6 mb-8 backdrop-blur-sm">
            <h2 className="text-base font-semibold text-slate-200 mb-4 flex items-center gap-2">
              {mode === 'single' && <FileText className="w-5 h-5 text-indigo-400" />}
              {mode === 'directory' && <FolderSearch className="w-5 h-5 text-indigo-400" />}
              {mode === 'zip' && <Upload className="w-5 h-5 text-indigo-400" />}
              <span>
                {mode === 'single' && 'Tek Dosya Analiz Girdisi'}
                {mode === 'directory' && 'Yerel Klasör Yolu İle Tarama'}
                {mode === 'zip' && 'ZIP Proje Yükleme'}
              </span>
            </h2>
                ...
                {singleInputType === 'upload' ? (
                  <div className="border-2 border-dashed border-slate-800 hover:border-indigo-500/50 rounded-xl p-6 text-center transition bg-slate-950/40">
                    ...
                    <label htmlFor="single-file-input" className="cursor-pointer flex flex-col items-center gap-2">
                      ...
                      <span className="text-xs text-slate-500">Desteklenen: .py, .c, .cpp, .java</span>
                    </label>
                  </div>
                ) : (
                  <div>
            ...
            {mode === 'directory' && (
              <div>
                <label className="text-xs text-slate-400 block mb-2">Bilgisayarınızdaki Klasör Yolu:</label>
                <input
                  type="text"
                  value={dirPathInput}
            ...
            {mode === 'zip' && (
              <div className="border-2 border-dashed border-slate-800 hover:border-indigo-500/50 rounded-xl p-6 text-center transition bg-slate-950/40">
                ...
                <label htmlFor="zip-file-input" className="cursor-pointer flex flex-col items-center gap-2">
                  <Upload className="w-8 h-8 text-slate-500" />
                  <span className="text-sm font-medium text-slate-300">
                    {zipFile ? zipFile.name : 'ZIP Dosyası Seçin'}
                  </span>
            ...
            {errorMsg && (
              <div className="mt-4 bg-red-500/10 border border-red-500/30 rounded-xl p-3 text-xs text-red-400 flex items-center gap-2">
            ...
            <div className="mt-6 flex justify-end">
              <button
                ...
                disabled={loading}
                className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-medium text-sm px-6 py-2.5 rounded-xl shadow-lg shadow-indigo-600/20 transition disabled:opacity-50"
              >
                {loading ? (
                  <>
          ...
          {!singleResult && !dirResult && !loading && (
            <div className="border border-slate-800/80 bg-slate-900/30 rounded-2xl p-12 text-center my-6 backdrop-blur-sm">
              ...
              <p className="text-sm text-slate-400 max-w-lg mx-auto mb-8 leading-relaxed">
                Sol menüden tek bir dosya yükleyin, projenizin yerel klasör yolunu yapıştırın veya ZIP yükleyip <b>🔍 Analiz Et</b> butonuna tıklayın.
              </p>
